...
class FileWriter:
    ...
    def __init__(self, tFname, mFname, kFname, kinetics=False):
    ...
    def openOutputFiles(self, params):
        ...
        if self.kinetics:
            self.kFout = open(self.kFname, "w")
            self.writeKineticsHeader(params)
    ...
    def writeKineticsHeader(self, params):
        
        # Here I am using formatted output to write header data to files
        # '\n' is newline
        self.kFout.write("# MS LJGAS KINETICS #\n")
        
        # '%s' means I'm writing a string. The string is specified in brackets at the end
        # '%d' means I'm writing an integer. The integer is specified in brackets at the end
        self.kFout.write("Simulation Type, %s\n" % (params.simulationType))
        self.kFout.write("numSprings, %d\n" % (params.numSprings))
    ...
    def writeMeasurementHeader(self, params):
        ...
        self.mFout.write("Time,CentroidX,CentroidY,CentroidZ,ElasticEnergy,LJEnergy")
    ...
    def writeFrame(self, pSys, box, params, simTime):
        ...
        # Then kinetics
        if self.kinetics:
            self.writeKineticsFrame(pSys, params, simTime)
    ...
    def writeKineticsFrame(self, pSys, params, simTime):
        
        # Kinetics is just a big list of all spring states
        # Make it so it's easy to read back in
        ...
        # All states on a single line
        self.kFout.write("%d" % (pSys.spring[0].currentState))
        for s in range(1, params.numSprings):
            self.kFout.write(",%d" % (pSys.spring[s].currentState))
        self.kFout.write("\n")
    ...
    def writeMeasurementFrame(self, pSys, box, params, simTime):
        ...
        elasticEnergy = pSys.getElasticEnergy() # Pre-calculated in the main simulation loop
        ljEnergy = box.getLJEnergy() # This was precalculated in the big box loop to save time

        self.mFout.write("%.5f,%.5f,%.5f,%.5f,%.5f,%.5f" % (simTime, centroid[0], centroid[1], centroid[2], elasticEnergy, ljEnergy))
        if params.simulationType == "Langevin":
