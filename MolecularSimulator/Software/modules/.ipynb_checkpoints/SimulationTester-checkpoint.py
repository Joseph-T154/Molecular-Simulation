...
class SimulationTester():
    ...
    def testThermalEquilibration(self, pSys, params):
        ...
        # Load measurement file conditionally
        meas = None
        if pSys.numSprings != 0 or params.simulationType == "Langevin":
            meas = MDLJMeasurement()
            meas.load(params.mFname_Equilibration)

            # Get thermal equilibration limits
            start = -1
            end = -1
        ...
        # Elastic energy figure
        if pSys.numSprings != 0:
            ...
            if start != -1:
                ...
                # Plot measured
                plt.plot(meas.time[start:end], meas.elasticEnergy[start:end], "-bx", label="Elastic Energy")

                # Plot expected (half kT per degree of freedom)
                xVals = np.linspace(0,plt.xlim()[1],100)
                yVals = 0.5 * params.kbt * params.numSprings * np.ones(100, dtype="float")
        ...
        if params.simulationType == "Langevin":
            ...
            if start != -1:
                ...
                # Plot measured
                plt.plot(meas.time[start:end], meas.kineticEnergy[start:end], "-gx", label="Kinetic Energy")
        ...
        if pSys.numSprings != 0 or params.simulationType == "Langevin":
            ...
            oFname = os.path.splitext(params.mFname_Equilibration)[0] + "_ThermalEquilEnergy.png"
            plt.savefig(oFname, dpi=100)            
